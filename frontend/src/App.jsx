...
function Sidebar({ activeMenu, setActiveMenu, isLoggedIn, onRequireLogin, onOpenLogin, onLogout }) {
  const menus = [
    ...
    { id: "reviews", label: "My Reviews", icon: MessageSquareText, protected: true },
      ...
      <div className="w-full max-w-md rounded-[2rem] bg-white p-7 shadow-2xl">
        <div className="mb-7 flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-500">Fork & Cup 서비스를 이용해보세요.</p>
          </div>
        ...
        <label className="mb-4 block">
          <span className="mb-2 block text-sm font-black text-zinc-700">Email</span>
        ...
        <label className="mb-6 block">
          <span className="mb-2 block text-sm font-black text-zinc-700">Password</span>
          <div className="flex h-12 items-center gap-3 rounded-2xl border border-zinc-200 bg-zinc-50 px-4 focus-within:border-orange-500 focus-within:bg-white">
...
function LoginRequiredPopup({ open, onClose, onOpenLogin }) {
  if (!open) return null;
    ...
    <div className="absolute inset-0 z-[780] grid place-items-center bg-zinc-900/20 px-4 backdrop-blur-sm">
      <div className="w-full max-w-sm rounded-[2rem] bg-white p-7 text-center shadow-2xl">
        ...
        <div className="mt-7 grid grid-cols-2 gap-3">
          <button onClick={onClose} className="rounded-2xl bg-zinc-100 px-4 py-3 text-sm font-black text-zinc-500 transition hover:bg-zinc-200">
            닫기
          </button>
          <button
            onClick={() => {
